...
best_img_path_global = {}  # for web suggestions
compatibility_feedback = None  # thumbs up/down for model suggestion
web_feedbacks = {}  # dictionary for thumbs up/down for each web suggestion
...
body = dbc.Row([
    ...
    dbc.Col(html.Div([
        ...
        dbc.Card(dbc.CardBody([
         ...
         html.Div([
                 # ⭐ ADD this for feedbacks
            html.Div(id="compatibility-feedback", style={"textAlign": "center", "marginTop": "20px"}),
            html.Div(id="web-feedbacks-section", style={"marginTop": "30px"}),
...
def update_output(n_clicks, try_most, top, bottom, shoe, bag, accessory, top_style, bottom_style, shoe_style, bag_style, accessory_style):
    ...
    if n_clicks:
        ...
        global compatibility_feedback
        compatibility_feedback = None 
...
def show_web_suggestions(n_clicks, selected_category):
    ...
    for idx, item in enumerate(suggestions):
        web_items.append(html.Div([
            ...
            html.Div([
                dbc.Button("👍", id={"type": "web-thumb-up", "index": idx}, color="success", size="sm", style={"margin": "2px"}),
                dbc.Button("👎", id={"type": "web-thumb-down", "index": idx}, color="danger", size="sm", style={"margin": "2px"}),
            ], style={"textAlign": "center"})
        ], style={"display": "inline-block", "margin": "10px"}))

    return html.Div(web_items, style={"display": "flex", "flexWrap": "wrap", "justifyContent": "center"})


@app.callback(
    Output("compatibility-feedback", "children"),
    Input("submit-button", "n_clicks"),
    prevent_initial_call=True
)
def show_compat_feedback_buttons(n_clicks):
    return html.Div([
        html.H5("Did you like the model's outfit suggestion?", style={"color": "#00BFFF"}),
        dbc.Button("👍", id="compat-thumbs-up", color="success", style={"margin": "5px"}),
        dbc.Button("👎", id="compat-thumbs-down", color="danger", style={"margin": "5px"}),
    ], style={"textAlign": "center"})


@app.callback(
    Output("compat-thumbs-up", "children"),
    Output("compat-thumbs-down", "children"),
    Input("compat-thumbs-up", "n_clicks"),
    Input("compat-thumbs-down", "n_clicks"),
    prevent_initial_call=True
)
def handle_compat_feedback(up_clicks, down_clicks):
    global compatibility_feedback
    ctx = dash.callback_context.triggered[0]["prop_id"].split(".")[0]
    if ctx == "compat-thumbs-up":
        compatibility_feedback = "👍"
    elif ctx == "compat-thumbs-down":
        compatibility_feedback = "👎"
    return ("👍 (selected)" if compatibility_feedback == "👍" else "👍",
            "👎 (selected)" if compatibility_feedback == "👎" else "👎")

@app.callback(
    Output("web-feedbacks-section", "children"),
    Input({"type": "web-thumb-up", "index": dash.dependencies.ALL}, "n_clicks"),
    Input({"type": "web-thumb-down", "index": dash.dependencies.ALL}, "n_clicks"),
    prevent_initial_call=True
)
def handle_web_feedbacks(up_clicks, down_clicks):
    global web_feedbacks
    ctx = dash.callback_context.triggered[0]["prop_id"]
    if ".n_clicks" in ctx:
        ctx_id = json.loads(ctx.split(".")[0])
        idx = ctx_id["index"]
        if ctx_id["type"] == "web-thumb-up":
            web_feedbacks[idx] = "👍"
        elif ctx_id["type"] == "web-thumb-down":
            web_feedbacks[idx] = "👎"

    return html.P(f"Recorded feedbacks: {web_feedbacks}", style={"color": "#00BFFF"})
